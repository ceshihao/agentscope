...
class ReActAgent(ReActAgentBase):
    ...
    async def reply(
        ...
        """
        # Handle pending tool confirmation if any
        if self.toolkit.has_pending_confirmation():
            pending_confirmation = self.toolkit.get_pending_confirmation()
            if pending_confirmation and msg:
                # Check if user's response indicates confirmation
                user_text = ""
                if isinstance(msg, list):
                    # For msg_list, check the last message (user's latest input)
                    last_msg = msg[-1] if msg else None
                    if last_msg and last_msg.role == "user":
                        user_text = last_msg.get_text_content() or ""
                else:
                ...
                if any(keyword in user_text for keyword in ["yes", "y", "是", "确认", "允许", "同意", "ok", "okay", "好的", "可以"]):
                    # User confirmed, execute the tool directly and return
                    
                    # Clear pending confirmation BEFORE execution to prevent recursive confirmation
                    tool_name = pending_confirmation["tool_name"]
                    tool_args = pending_confirmation["tool_args"]
                    self.toolkit.clear_pending_confirmation()
                    
                    # Execute the tool directly without going through _acting (which would trigger confirmation again)
                    tool_func = self.toolkit.tools[tool_name]
                    try:
                        if inspect.iscoroutinefunction(tool_func.original_func):
                            result = await tool_func.original_func(**tool_args)
                        else:
                            result = tool_func.original_func(**tool_args)
                        ...
                        # Determine user language for result message
                        user_language = "English"  # Default
                        if isinstance(msg, list) and msg:
                            for m in reversed(msg[-10:]):  # Check last 10 messages
                        ...
                        # Return the execution result with Markdown formatting
                        ...
                        error_user_language = "English"  # Default
                        if isinstance(msg, list) and msg:
                            for m in reversed(msg[-10:]):  # Check last 10 messages
                                if hasattr(m, 'role') and m.role == "user":
                                    content = m.get_text_content() if hasattr(m, 'get_text_content') else str(m.content)
                                    if any('\u4e00' <= char <= '\u9fff' for char in content):
                                        error_user_language = "Chinese"
                                        break
                        elif hasattr(msg, 'role') and msg.role == "user":
                            content = msg.get_text_content() if hasattr(msg, 'get_text_content') else str(msg.content)
                            if any('\u4e00' <= char <= '\u9fff' for char in content):
                        ...
                        if error_user_language == "Chinese":
                            reply_msg = Msg(
                                self.name,
                                f"## ❌ 工具执行失败\n\n**工具名称**: `{tool_name}`\n\n**错误信息**:\n```\n{str(e)}\n```",
                                "assistant",
                    ...
                    # Record the tool execution result in memory
                    await self.memory.add(reply_msg)
                    return reply_msg
                elif any(keyword in user_text for keyword in ["no", "n", "否", "拒绝", "不允许", "不同意", "cancel", "不要", "不行"]):
                    # User rejected, clear pending confirmation and return rejection message
                    self.toolkit.clear_pending_confirmation()
                    
                    # Determine language for rejection message
                    rejection_user_language = "English"  # Default
                    if isinstance(msg, list) and msg:
                        for m in reversed(msg[-10:]):  # Check last 10 messages
                            if hasattr(m, 'role') and m.role == "user":
                                content = m.get_text_content() if hasattr(m, 'get_text_content') else str(m.content)
                                if any('\u4e00' <= char <= '\u9fff' for char in content):
                                    rejection_user_language = "Chinese"
                                    break
                    elif hasattr(msg, 'role') and msg.role == "user":
                        content = msg.get_text_content() if hasattr(msg, 'get_text_content') else str(msg.content)
                        if any('\u4e00' <= char <= '\u9fff' for char in content):
                    ...
                    if rejection_user_language == "Chinese":
                        reply_msg = Msg(
                            self.name,
                            f"## ❌ 工具执行已取消\n\n我理解您的决定，不会执行工具 `{pending_confirmation['tool_name']}`。",
                            "assistant",
                        )
                    else:
                        reply_msg = Msg(
                            self.name,
                            f"## ❌ Tool Execution Cancelled\n\nI understand. I will not execute the tool `{pending_confirmation['tool_name']}` as you requested.",
                            "assistant",
                        )
                    
                    await self.print(reply_msg, True)
                    # Record the tool cancellation result in memory
                    await self.memory.add(reply_msg)
                    return reply_msg
                else:
                    # Unclear response, ask for clarification
                    # Determine language for clarification message
                    clarification_user_language = "English"  # Default
                    if isinstance(msg, list) and msg:
                        for m in reversed(msg[-10:]):  # Check last 10 messages
                            if hasattr(m, 'role') and m.role == "user":
                                content = m.get_text_content() if hasattr(m, 'get_text_content') else str(m.content)
                                if any('\u4e00' <= char <= '\u9fff' for char in content):
                                    clarification_user_language = "Chinese"
                                    break
                    elif hasattr(msg, 'role') and msg.role == "user":
                        content = msg.get_text_content() if hasattr(msg, 'get_text_content') else str(msg.content)
                        if any('\u4e00' <= char <= '\u9fff' for char in content):
                    ...
                    if clarification_user_language == "Chinese":
                        reply_msg = Msg(
                            self.name,
                            f"## ❓ 需要明确确认\n\n我需要明确的确认。请回复 **'是'** 或 **'yes'** 来允许执行工具 `{pending_confirmation['tool_name']}`，或回复 **'否'** 或 **'no'** 来拒绝执行。",
                            "assistant",
                        )
                    else:
                        reply_msg = Msg(
                            self.name,
                            f"## ❓ Confirmation Required\n\nI need a clear confirmation. Please respond with **'yes'** to allow the tool `{pending_confirmation['tool_name']}` execution, or **'no'** to reject it.",
                            "assistant",
                        )
                    
                    await self.print(reply_msg, True)
                    # Record the clarification request in memory
                    await self.memory.add(reply_msg)
                    return reply_msg
    ...
    async def _acting(self, tool_call: ToolUseBlock) -> Msg | None:
        ...
        try:
            ...
            async for chunk in tool_res:
                ...
                if (
                    chunk.metadata 
                    and chunk.metadata.get("tool_confirmation_required")
                ):
                    # Return confirmation message to user
                    confirmation_msg = chunk.metadata.get("confirmation_message", 
                        f"I need to execute the tool '{tool_call['name']}' with parameters: {tool_call.get('input', {})}. Please confirm if I should proceed (yes/no).")
                    
                    # The pending confirmation is already set in call_tool_function
                    # It will be cleared in reply() method when user responds
                    return Msg(
                        self.name,
                        confirmation_msg,
                        "assistant",
